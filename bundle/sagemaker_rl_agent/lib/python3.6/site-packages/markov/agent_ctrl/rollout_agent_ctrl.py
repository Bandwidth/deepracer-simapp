...
class RolloutCtrl(AgentCtrlInterface, ObserverInterface, AbstractTracker):
    ...
    def _get_car_reset_model_state(self, car_pose):
        ...
        if closest_obstacle_pose is not None:
            ...
            cur_dist = closest_object_dist - self._reset_behind_dist
    ...
    def update_agent(self, action):
        ...
        try:
            ...
            origin = np.array([current_car_pose.position.x,
                               current_car_pose.position.y,
                               current_car_pose.position.z])
            translation = const.RELATIVE_POSITION_OF_FRONT_OF_CAR
            rotation = np.array([current_car_pose.orientation.x,
    ...
    def finish_episode(self):
        ...
        if self._start_pos_behavior_['change_start'] and self._is_training_:
            self._data_dict_['start_ndist'] = random.random() 
