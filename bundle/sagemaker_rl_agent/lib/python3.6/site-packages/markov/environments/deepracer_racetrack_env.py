    ...
    def _update_state(self):
        try:
            ...
            if (episode_name == None):
                episode_name = "e%d" % math.floor(time.time())
                os.makedirs("/images/" + episode_name, exist_ok=True)
            ...
            # Write out the camera view along with the position and heading
            agent_idx = 0
            params = self.agent_list[agent_idx]._ctrl_._reward_params_
            x = math.floor(params['x']*1000.0)/1000.0
            y = math.floor(params['y']*1000.0)/1000.0
            heading = math.floor(params['heading']*1000.0)/1000.0
            steps = int(params['steps'])
            
            # Record camera view
            if ('FRONT_FACING_CAMERA' in old_state[agent_idx]):
                image = Image.fromarray(np.asarray(old_state[agent_idx]['FRONT_FACING_CAMERA']))
                image.save('/images/{}/i{}_X{}_Y{}_H{}.jpeg'.format(episode_name,steps,x,y,heading), "JPEG")
            else:
                stereo = np.asarray(old_state[agent_idx]['STEREO_CAMERAS'])
                with open('/images/{}/i{}_X{}_Y{}_H{}_stereo.npy'.format(episode_name,steps,x,y,heading), 'wb') as f:
                    np.save(f, stereo)
            
            # Record LIDAR view
            if ('LIDAR' in old_state[agent_idx]):
                lidar = np.asarray(old_state[agent_idx]['LIDAR'])
                with open('/images/{}/i{}_X{}_Y{}_H{}_lidar.npy'.format(episode_name,steps,x,y,heading), 'wb') as f:
                    np.save(f, lidar)
