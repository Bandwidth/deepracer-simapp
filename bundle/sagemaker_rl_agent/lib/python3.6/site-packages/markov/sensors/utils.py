...
def add_random_noise(image, iteration):
    ...
    # No modification 10% of the time
    if False: # (random.random() < 0.1):
        return image

    # Save original image before modification
    if (iteration % 225 == 0):
        image.save('/images/{}.jpeg'.format(filename), "JPEG")
    ...
    if True: #(random.random() < 0.5):
        # Chop off top ~1/4 of screen (the wall and sky)
        shape = [(0, 0), (640, 140)]
        #color = (int(uniform(0.0,255.0)),int(uniform(0.0,255.0)),int(uniform(0.0,255.0)))
        color = (128,128,128)
        draw = ImageDraw.Draw(image)
        draw.rectangle(shape, fill=color)   # Fill with a random color

    if False: #(random.random() < 0.5):
        # ---- draw a bunch of circles and blur them ----
        draw = ImageDraw.Draw(image)

        # Generate a basic random colour
        delta = 80

        R, G, B = random.randint(delta,255-delta), random.randint(delta,255-delta), random.randint(delta,255-delta),

        min_diam = 20
        max_diam = 80
        min_rad = int(min_diam/2)
        max_rad = int(max_diam/2)

        incr = random.randint(min_diam,max_diam)
        for _ in range(0,200): # range(cmin,cmax):
            # Choose RGB values for this circle around the basic RGB color
            r = R + random.randint(-delta,delta)
            g = G + random.randint(-delta,delta)
            b = B + random.randint(-delta,delta)
            diam = random.randint(min_diam, max_diam)
            x = random.randint(-min_rad, screen_width + min_diam)
            y = random.randint(-min_rad, wall_cutoff  - max_rad)
            draw.ellipse([x,y,x+diam,y+diam], fill=(r,g,b))

    if True: #(random.random() < 0.5):
        # Rotate the image
        image = image.rotate(uniform(-3.0,3.0))

    if True: #(random.random() < 0.5):
        # Blur the image
        image = image.filter(ImageFilter.GaussianBlur(uniform(2.0,4.0)))

    if True: #(random.random() < 0.5):
        # Add noise to the image
        shape = (image.height,image.width,3) # NumPy stores indices backwards..
        wd = uniform(0.3,0.8)
        gauss = np.random.normal(0.0, wd, shape)
        gauss = gauss.reshape(shape).astype('uint8')
        image = Image.blend(image,Image.fromarray(gauss),wd/2.5)  # Favor the original image

    if (iteration % 225 == 0):
        image.save('/images/{}_random.jpeg'.format(filename), "JPEG")

    return image


    # Clip and zoom the image to remove black areas caused by rotation
    # image_array = np.asarray(pil_image)
    # image_array = rotate_image(image_array, random_degrees)
    # image_array = crop_around_center(
    #     image_array,
    #     *largest_rotated_rect(
    #         pil_image.width,
    #         pil_image.height,
    #         math.radians(random_degrees)
    #     )
    # )                   
