...
def add_random_noise(image, iteration):
    ...
    Parameters:
        image (PIL Image): The PIL image to add random noise to.
        iteration (int): current iteration count. 
    Returns:    
        PIL Image: A PIL image with noise added
    """
    # determine if we need to save images or not
    save_images = False
    if (iteration % 225): 
        save_images = True
    ...
    if 'random' in randomness:
        ...
        random_noise_image = cv2.add(cv_image, gauss)
        # rotate image some degrees (-3 to 3 degrees)
        random_degrees = random.randint(-3, 3)
        image_height, image_width = random_noise_image.shape[0:2]
        random_noise_image = _rotate_image(random_noise_image, random_degrees)
        random_noise_image = _crop_around_center(
            random_noise_image,
        ...
        # save images only if save_images == True
        if save_images == True:
            # write original image to disk
            cv2.imwrite('/images/{}.jpeg'.format(filename), cv_image)
            # write random noise image to disk
            cv2.imwrite('/images/{}_{}.jpeg'.format(filename, randomness[0]), random_noise_image)
        
        # convert OpenCV2 Image to PIL Image
        image = Image.fromarray(cv2.cvtColor(random_noise_image, cv2.COLOR_BGR2RGB))
