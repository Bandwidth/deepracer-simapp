...
def add_random_noise(image):
    ...
    Parameters:
        image (ROS Image): The ROS image to add random noise to.

    Returns:
        ROS Image: The original ROS image(s) with noise added
    '''
    ...
    try:
        ...
        LOG.info("[Sensor Utils]: ROS image message to cv2 error: {}".format(ex))
    ...
    cv2.imwrite('/image_data/{}.jpeg'.format(filename), cv_image)
    ...
    try:
        image = bridge.cv2_to_imgmsg(random_noise_image, "bgr8")
    except CvBridgeError as ex:
        LOG.info("[Sensor Utils]: cv2 image to ROS image message error: {}".format(ex))
