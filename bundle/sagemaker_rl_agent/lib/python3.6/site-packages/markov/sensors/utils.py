...
""" def add_random_noise(images):
    '''Adds random noise to image(s)

    Parameters:
        images (tuple): The ROS image(s) to add random noise to. At least 1 ROS image is expected 

    Returns:
        tuple(): The original ROS image(s) with noise added
    '''

    # available choices for random noise addition to image. orignal means no randomness will be added
    choices = ['original', 'speckle']
    # choose a random choice
    randomness = random.choices(choices, weights = [1, 1], k=1)
    # exit early if randomness is not needed
    if randomness == 'original':
        return images
    # generate random file name based on uuid
    filename = uuid.uuid()    
    # convert ROS Image to OpenCV2 Image
    cv2_image = bridge.imgmsg_to_cv2(images[0], "rgb8")
    # add noise to the image
    if 'speckle' in randomness:
        gauss = np.random.normal(0, 1, cv2_image.size)
        gauss = gauss.reshape(cv2_image.shape[0], cv2_image.shape[1], cv2_image.shape[2]).astype('uint8')
        random_noise_image = cv2.add(cv2_image, gauss)
    # write original image to disk
    cv2.imwrite('/image_data/{}.jpeg'.format(filename), cv2_image)
    # write random noise image to disk
    cv2.imwrite('/image_data/{}_{}.jpeg'.format(filename, randomness), random_noise_image)
    # convert OpenCV2 Image to ROS Image
    image = bridge.cv2_to_imgmsg(random_noise_image, encoding="passthrough")
